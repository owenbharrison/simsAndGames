		...
		int clipAgainstPlane(const vf3d& ctr, const vf3d& norm, Triangle& a, Triangle& b) const {
			...
			switch(in_ct) {
				...
				case 1: {
					...
					a.t[1]=*in_tex[0]+t*(*out_tex[0]-*in_tex[0]);

					t=0;
					a.p[2]=segIntersectPlane(*in_pts[0], *out_pts[1], ctr, norm, &t);
					//interpolate tex coords
					a.t[2]=*in_tex[0]+t*(*out_tex[1]-*in_tex[0]);
				...
				case 2: {
					...
					//interpolate tex coords
					a.t[2]=*in_tex[1]+t*(*out_tex[0]-*in_tex[1]);
					...
					//interpolate tex coords
					b.t[2]=*out_tex[0]+t*(a.t[0]-*out_tex[0]);
