/*TODO:
* make projectAndClip based on current draw target?
*		how to reallocate buffers & index function
*		how to update projection matrix?
*		like opengl viewport command
* add cam_near and cam_far options?
*/
...
namespace cmn {
	...
	public:
		...
		void projectAndClip() {
			...
			{
				mat4 look_at=mat4::makeLookAt(cam_pos, cam_pos+cam_dir, cam_up);
			...
			for(const auto& tri:tris_to_project) {
				...
				if(norm.dot(tri.p[0]-cam_pos)<0) {
					...
					for(int i=0; i<3; i++) {
						float w=1;
						tri_view.p[i]=matMulVec(cam_view, tri.p[i], w);
					...
					for(int i=0; i<num; i++) {
						...
						for(int j=0; j<3; j++) {
							//project
							float w=1;
							tri_proj.p[j]=matMulVec(cam_proj, clipped[i].p[j], w);
			...
			for(const auto& line:lines_to_project) {
				...
				for(int i=0; i<2; i++) {
					float w=1;
					line_view.p[i]=matMulVec(cam_view, line.p[i], w);
				...
				if(line_view.clipAgainstPlane(vf3d(0, 0, -near_plane), vf3d(0, 0, -1), clipped)) {
					...
					for(int j=0; j<2; j++) {
						//project
						float w=1;
						line_proj.p[j]=matMulVec(cam_proj, clipped.p[j], w);
...
#pragma region RENDER HELPERS
		void FillDepthTriangle(
			...
			if(dy1) dax_step=float(dx1)/std::abs(dy1);
			if(dy2) dbx_step=float(dx2)/std::abs(dy2);

			if(dy1) dw1_step=float(dw1)/std::abs(dy1);
			if(dy2) dw2_step=float(dw2)/std::abs(dy2);
		...
		void FillTexturedDepthTriangle(
			...
			if(dy1) du1_step=float(du1)/std::abs(dy1);
			if(dy1) dv1_step=float(dv1)/std::abs(dy1);
			if(dy1) dw1_step=float(dw1)/std::abs(dy1);
			if(dy2) du2_step=float(du2)/std::abs(dy2);
			if(dy2) dv2_step=float(dv2)/std::abs(dy2);
			if(dy2) dw2_step=float(dw2)/std::abs(dy2);
...
#pragma endregion
	...
	bool Engine3D::OnUserCreate() {
		...
		float asp=ScreenWidth()/float(ScreenHeight());
		cam_proj=mat4::makePerspective(cam_fov_deg, asp, near_plane, far_plane);
