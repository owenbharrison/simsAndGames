...
void main() {
	...
	//sample image at same spot across character
	vec2 trunc_uv=floor(div)*8/resolution;
	
	//make sure not to miss any important pixels
	//so grab the maximum pixel in the 8x8 region
	vec4 sum=vec4(0);
	for(int i=0; i<8; i++) {
		for(int j=0; j<8; j++) {
			vec2 off=vec2(i, j)/resolution;
			sum+=texture(tex1, trunc_uv+off);
		}
	}
	sum/=64;
	float lum=dot(sum.rgb, vec3(.299, .587, .114));
	
	//determine char to use.
	int pal_ix=int(float(palette.length())*lum);
	int letter=palette[pal_ix];
	...
	//saturated color?
	if(bit!=0) pix_out=vec4(sum.rgb/lum, 1);
